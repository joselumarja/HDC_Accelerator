...
//
// Wrapper para adaptar hdc_accelerator_top, con puertos OBI
// planos, a interfaces OBI basadas en structs:
//
//   obi_pkg::obi_req_t
//   obi_pkg::obi_resp_t
//
// El acelerador expone:
//   - un puerto OBI slave  para configuración
//   - un puerto OBI master para accesos a memoria
// ============================================================
...
module hdc_accelerator_obi_wrapper #(
    ...
    // Usado para configurar registros internos:
    // addr_A, addr_B, addr_C, vector sizes, sel_op, start
    // --------------------------------------------------------
    ...
    // Usado para leer A/B y escribir C
    // --------------------------------------------------------
    ...
    logic                  slv_obi_req;
    logic                  slv_obi_we;
    logic [ADDR_WIDTH-1:0] slv_obi_addr;
    logic [DATA_WIDTH-1:0] slv_obi_wdata;
    logic [DATA_WIDTH-1:0] slv_obi_rdata;
    logic                  slv_obi_gnt;
    ...
    logic                  mst_obi_req;
    logic                  mst_obi_we;
    logic [ADDR_WIDTH-1:0] mst_obi_addr;
    logic [DATA_WIDTH-1:0] mst_obi_wdata;
    logic [DATA_WIDTH/8-1:0] mst_obi_be;
    logic                  mst_obi_gnt;
    logic [DATA_WIDTH-1:0] mst_obi_rdata;
    ...
    assign slv_obi_addr  = hdc_slave_req_i.addr[ADDR_WIDTH-1:0];
    assign slv_obi_wdata = hdc_slave_req_i.wdata[DATA_WIDTH-1:0];
    ...
    // Ajustar si tu obi_pkg::obi_resp_t tiene nombres distintos.
    assign hdc_slave_resp_o.err    = 1'b0;
    ...
    assign mst_obi_rdata  = hdc_master_resp_i.rdata[DATA_WIDTH-1:0];

    // Campos adicionales de petición.
    // Estos campos pueden variar según la definición concreta de obi_pkg.
    // Si alguno no existe en tu obi_pkg, elimínalo.
    assign hdc_master_req_o.aid     = '0;
    assign hdc_master_req_o.mid     = '0;
    assign hdc_master_req_o.auser   = '0;
    assign hdc_master_req_o.wuser   = '0;
    assign hdc_master_req_o.atop    = '0;
    assign hdc_master_req_o.memtype = '0;
    assign hdc_master_req_o.prot    = '0;

    // ========================================================
    // Instancia del acelerador
    // ========================================================
    ...
    hdc_accelerator_top #(
        ...
        // Debug
        // ----------------------------------------------------
        .done_debug (done_debug_o),
        .state_debug(state_debug_o)
    );
